...
| ---------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Source     | Pair text in `claude/output-styles/pair.md` and the Codex `developer_instructions`, identical. Roles explore, implement, browser, git, general, identical per harness. |
...
| Unverified | No real thread has run the 2026-09-24 version: command timeout, pipeline wait by the pair, done pass at the gate, cached browser runner.                               |
...
| Browser   | rendered criteria and observed defects or pass                                                          | implementation                                                     |
...
| Browser reuses one resolved runner and one login                                     | An 84-minute `npx --yes agent-browser` run had a 40 s median call against 0.4 s with `bunx`; warm overhead per call: npx 5.6 s, vpx 1–1.7 s, bunx 0.3 s     |
| Prototypes are shown by screenshots and deleted by the first execution brief         | Variants outlived the user's pick behind a hung slice; five prototype acceptance runs with fix loops used about 50M tokens                                  |
| The gate agent makes the done pass over the whole branch diff                        | A finished model-picker MR still had about 30% removable code: duplicated helpers, derivable fields, prototype leftovers                                    |
