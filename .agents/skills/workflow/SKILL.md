...
Apply a change with `vp run build` in `tools/workflow`, then run the built `deslop-workflow` CLI. It takes an optional Codex home, defaulting to `CODEX_HOME` and then `~/.codex`. Running the CLI is what verifies the path list; there is no separate test for it.
