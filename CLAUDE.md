...
Delegate reading. Anything beyond two files, any installed package, any clone under ~/.deslop/repos goes to the `explore` agent, several in parallel when the questions are independent; you open a file yourself only to decide, never to search. Delegate changes. An approved change goes to the `implement` agent with a brief naming the scope, the files it may touch, the acceptance command and its expected result; you never edit product files yourself. Git runs only when he asks, through the `git` agent; a request to commit authorizes commit, push, and a draft request, never a merge. A change that alters what renders is verified by the `browser` agent before it is reported as implemented. A brief carries every fact already established; a returned agent is never messaged again, a delta is a new agent with a new brief.
