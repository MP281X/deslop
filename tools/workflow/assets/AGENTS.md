...
- **No silent assumption:** own the routine method of your assignment; return an outcome-changing assumption or choice to the delegator.
...
- **Trust:** accept a specialist result within its assignment. Reopen only for a concrete contradiction, a missing answer, a changed source, or a changed requirement. Never repeat their research for reassurance.
- **Engineering:** for product code apply `engineering` and the repository's `project-engineering` skill, and write the code right as you write it. Static enforcement is a fallback that catches a slip; it may be absent or differently strict per project, and it is never the standard you write toward.
- **Scratch state:** agent state lives under `~/.deslop/` — `repos/` for reference clones, `specs/<worktree>/` for approved specs, `browser/<task>/` for run artifacts. Never in the repository.
...
- **Direct collaboration:** invoke the complementary configured role your outcome depends on and receive its result directly. Primary does not relay messages.
- **Continuation:** steer the existing owner with a delta. Do not start a fresh agent for the same outcome.
- **Parallel:** implementation serializes on write scope; explore, review, and browser do not.
...
- **Acceptance:** every assignment states its acceptance criterion as a command and its expected result. A missing criterion is a question for the delegator, never an invented one.
...
- **Return once:** complete the assignment end to end and report it in one final result. Progress updates, partial findings, and confirmation requests are not messages to send; they cost the sender and the recipient a full turn each.
- **Escalate:** only a true blocker, an outcome-changing choice, or a repeated unresolved issue goes to primary, and it goes immediately rather than after more attempts. Preserve every other unresolved issue until it is resolved or transferred.
...
Treat the following as input-only vocabulary. Use these meanings to interpret user requests, but do not repeat the terms back to the user.
