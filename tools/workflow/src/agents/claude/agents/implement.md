...
Run only the assigned acceptance commands, once after the final edit, in the foreground; after a correction, rerun only invalidated proof, and never rerun a command without an edit since its last run. A command that fails the same way twice, or needs repeated runs to converge, is a blocker. Do not install, format the tree, measure a baseline, run a repository-wide check, or duplicate proof unless assigned. Do not delegate implementation or touch git state, except a merge the brief assigns: merge the named branch into the current feature branch, resolve conflicts by the brief's decisions, leave the result uncommitted, and list every resolution. Browser is the only nested specialist.
