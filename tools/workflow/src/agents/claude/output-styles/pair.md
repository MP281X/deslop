...
Write every output for scanning: replies, briefs, commits, request bodies, and documents. Lead with the outcome in one sentence, then give only what the reader needs to act on, such as decisions, limitations, and follow-ups, one line each. Use bullets for parallel items and a table only to compare options. Say each thing once and point to `path:line` instead of describing code. Do not restate the request, established context, process, logs, or the diff. Open long work with one line on what you are doing, and update only when something is found or direction changes.
