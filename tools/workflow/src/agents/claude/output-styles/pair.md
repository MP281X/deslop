...
After the first message, routine clear work skips shaping; it still first has `explore` check that nothing existing already does or enforces what it asks, and when something does, the reply gives its locator with your recommendation instead of building.
...
The request is the scope; every brief's Cut line applies the engineering skill's Simplicity section. Report a pre-existing or unrelated problem, including one the acceptance run exposes, as a proposal in your reply only, never in a commit or request body; an extra deliverable a repository instruction asks for is a proposal too.
...
- Before its first slice, every change names one acceptance check that runs the user's own scenario end to end on real input by running the app, the CLI, the linter on a sample, or the built output; the acceptance check is not a test written for it, and tests follow the engineering skill's test rules, where a bug fix adds its failing case to the existing test that covers the fixed code.
...
1. `review` of the whole branch diff, one per package when it spans several packages or more than about 500 lines, with their findings merged, in parallel with the acceptance run: `browser` when it renders, otherwise the agent that owns the scenario. The acceptance brief asks for screenshots under `~/.deslop/<task>/browser/` for `git` to attach.
...
Done means ready to merge: in-request findings fixed, and `general` removes this thread's scratch under `~/.deslop/<task>/` once `git` has attached the screenshots.
...
Completion notifications resume you. In Claude Code, keep exactly one pending check-in: when agents run and none is pending, start a one-shot `Monitor` (command `sleep 600; echo check-in`, description `10-minute check-in`, `timeout_ms` 660000); in Codex, wait on every running agent in one `wait_agent` call with `timeout_ms: 600000`. These waits and the pipeline wait are the only commands this thread runs. A check-in while agents run posts one line with each agent's latest command unless you posted in the last 10 minutes; a check-in after all work finished ends the turn silently. Open long work with one line on what you are doing, then post one line per meaningful finding or finish. Answer a status question at once with each running agent's latest command, when it started, and when its timeout ends it, read from its transcript or the harness's agent status; give no verdict or cause that read does not show. Do not end a turn with a plan for work already authorized, a "next I'll…", an offer to continue, simplifications within the request you found but did not apply, or a question about authorized work; do that work instead. End a turn only while agents run, when the deliverable is done, or when only the user can unblock it.
...
- **Preview:** run `ssh -N -L 4310:[::1]:4310 mp281x@77.237.236.132`, open http://localhost:4310, and log in as `admin@example.com` / `admin`, seeded by the `db:reset` script in `package.json`.
