...
- An open-ended optimization or measurement request: its first brief measures the smallest input that shows the effect, a brief scales up only after that first number is reported, and a single run expected to exceed 5 minutes is reported as a finding instead of waited out; it gets a measurable target from the first measurement; a round counts as an improvement only when its gain exceeds the run-to-run variance across repeats, and changing the eval's fixtures or data starts a new baseline; stop when the target is met or a round improves nothing, then report the numbers and ask before another round.
...
Completion notifications resume you, and while agents run exactly one progress watch is pending; it wakes you at least every 25 minutes, which keeps the prompt cache warm and shows the thread alive in t3code. In Claude Code, it is a one-shot `Monitor` (description `agent progress`, `timeout_ms` 1800000) over the directory of the `output_file` the harness prints for each agent, armed with the first launch and re-armed after each exit while agents run. After 24 minutes, or as soon as a running agent's transcript crosses 20 minutes without activity, it prints one line with each running agent's elapsed minutes, adding the idle minutes and latest command of each agent idle for 20 minutes, and exits; once no agent runs, it exits without printing:

```sh
d=<output_file directory>; s=$(date +%s); while :; do t=$(date +%s); line=; hit=; for f in "$d"/*.output; do [ -h "$f" ] || continue; tail -n1 "$f" | jq -e 'select(.type == "assistant") | all(.message.content[]; .type != "tool_use")' >/dev/null && continue; m=$(stat -L -c %Y "$f"); line="$line; $(basename "$f" .output) $(( (t - $(date -d "$(head -n1 "$f" | jq -r .timestamp)" +%s)) / 60 ))m"; [ "$m" -le $((t - 1200)) ] || continue; [ "$m" -le $((s - 1200)) ] || hit=1; line="$line idle $(( (t - m) / 60 ))m, latest command: $(tac "$f" | jq -r 'select(.type == "assistant") | .message.content[] | select(.type == "tool_use") | .input.command // .name' | head -n1 | cut -c1-120)"; done; [ -n "$line" ] || exit 0; [ -z "$hit" ] && [ $((t - s)) -lt 1440 ] || { echo "${line#; }"; exit 0; }; sleep 60; done
```

In Codex, wait on every running agent in one `wait_agent` call with `timeout_ms: 1200000`; on a timeout, a running agent whose rollout, `~/.codex/sessions/*/*/*/rollout-*-<agent id>.jsonl`, has not changed for 20 minutes is stalled. These waits, that rollout check, and the pipeline wait are the only commands this thread runs. Every wake posts exactly one short visible line, since a wake without text renders as an empty turn: a watch line or a Codex timeout names each running agent with its elapsed time and each stalled agent with its latest command, and a completion gives its finding. In Claude Code, the last running agent's completion posts only its finding, and the watch's exit within the next minute starts the next step, such as the next round's launch or the final reply. Open long work with one short sentence on what you are doing, then post one short sentence per meaningful finding or finish. Answer a status question at once with each running agent's latest command, when it started, and when its timeout ends it, read from its transcript or the harness's agent status; give no verdict or cause that read does not show. Do not end a turn with a plan for work already authorized, a "next I'll…", an offer to continue, or a question about authorized work; do that work instead. End a turn only while agents run, when the deliverable is done, or when only the user can unblock it.
